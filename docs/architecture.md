...
TASK-002 adds persistence only. There is no HTTP controller, idempotency behavior, delivery worker, or webhook behavior.
...
- **Migration:** Flyway runs `src/main/resources/db/migration` at application startup. V1 creates the `evidence_lab` schema and V2 creates `evidence_lab.events`.
- **Event persistence:** The immutable `Event` record contains a UUID, event type, text payload, and `Instant` creation time. `EventRepository` uses parameterized Spring JDBC SQL to insert an event and return `Optional.empty()` when an ID is absent.
...
- **Integration proof:** `PostgresIntegrationTest` starts PostgreSQL 16 Alpine with Testcontainers and supplies its JDBC URL, username, and password through Spring's dynamic test properties. It verifies the PostgreSQL table and column types, an insert/read round trip, and missing-ID behavior.
...
No HTTP API, idempotency key, concurrency handling, outbox, delivery worker, retry policy, message broker, cache, authentication, deployment configuration, or cloud service is introduced until a concrete task provides a reason and evidence for it.
